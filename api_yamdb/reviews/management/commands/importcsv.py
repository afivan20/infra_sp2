...
from reviews.models import Category, Comment, User, Title, Review, Genre, GenreTitle
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for row in reader:
            Title.objects.create(id=row['id'], name=row['name'], year = row['year'], category_id = row['category'])
        ...
        for row in reader:
            ...
            Review.objects.create(id=row['id'], title_id=row['title_id'], text = row['text'], author_id = row['author'], pub_date = row['pub_date'], score=row['score'])
    
    

        comments = csv.DictReader(open(comments))
        for row in comments:
            Comment.objects.create(id=row['id'], review_id=row['review_id'], text = row['text'], author_id = row['author'], pub_date = row['pub_date'])
